 ...
 * 15/01/08
 * 
 * WeakClassifier, WeakClassifierGeneratorを継承したクラスを作って使用する
 * OnlineBoostingがGeneratorから生成したWeakClassifierを基にアンサンブルを構築する
 **/
...
public:
	 ...
	 *
	 * generator           : 弱識別器生成器
	 * selector_num        : アンサンブルに含まれるセレクタ数
	 * weak_classifier_num : 各セレクタに含まれる弱識別器数 (アンサンブル全体で selector_num * weak_classifier_num 個の弱識別器が保持される)
	 * init_labels         : 初期生成に用いるラベルデータ
	 * init_samples        : 初期生成に用いるサンプルデータ
	 * replace_cycle       : 弱識別器の置換えを行う周期，1以下で毎回置き換えを行う
	 * sample_keep_num     : 弱識別器生成用に保持するサンプル数
	 *************************************/
